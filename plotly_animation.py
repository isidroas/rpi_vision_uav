...
df = pd.read_csv('build/log.csv')
low = df['px'].tolist()
high = np.array(df['py']) # artificially added 20 to get the second graph above the first one
df.Date=df['t']

trace1 = go.Scatter(x=df['t'][:2],
                    y=low[:2],
                    mode='lines',
                    line=dict(width=1.5))

trace2 = go.Scatter(x = df['t'][:2],
                    y = high[:2],
                    mode='lines',
                    line=dict(width=1.5))
